...
int main(){
    int i = 0;
    int alocamento = 5; // numero para realocamento 
    int contadorAlocamento =0; // conta ate 5 para realocar mais memoria 
    int totalServidores =0;  // conta quantas vezes ja foi registrado 

    //alocando dinamicamente a struct notificao
    Notificacao *ptrNotificacoes  = (Notificacao *) malloc(sizeof(Notificacao)*alocamento);
    ...
    */
    ...
    while (1){
        //tem que ficar antes de receber um novo valor 
        //adiciona 5 espaços no aocamento caso chegue em 5 
         if (contadorAlocamento == 5){
            ptrNotificacoes = realloc(ptrNotificacoes, sizeof(Notificacao)*alocamento);
            contadorAlocamento =0;
        }//if
        ...
        //realoca para o numero total de servidores e finaliza 
        if (strcmp(ptrNotificacoes[i].pais, "fim") == 0){
            ptrNotificacoes = realloc(ptrNotificacoes, sizeof(Notificacao)*totalServidores); // realoca para o somente o numero usado 
            break;                                                                          //para o while quando receber (FIM)
        }//if
        
        //coletra ps dados 
        setbuf(stdin,NULL);
        printf("digite a cidade:");
        fgets(ptrNotificacoes[i].cidade,20,stdin);
        ptrNotificacoes[i].cidade[strcspn(ptrNotificacoes[i].cidade,"\n")] = '\0';
       
        setbuf(stdin,NULL);
        printf("digite os servidores UP:");
        scanf("%i",&ptrNotificacoes[i].serverUP);

        setbuf(stdin,NULL);
        printf("digite os servidores DOW:");
        scanf("%i",&ptrNotificacoes[i].serverDOW);

        totalServidores ++;// contador total alocados 
        contadorAlocamento++;// contador para a realocar 
        i++;  // contador para a posicao do ptr 
    }//while
...
*/
    FILE *arquivo = fopen("notifications.dat", "wb");//abre o arquivo no modo de escrita binaria 
    ...
    fwrite(ptrNotificacoes,sizeof(Notificacao))
    
    
    
    /*
    //for para salvar os dados dos servidores no arquivo binario 
    for (int i = 0; i < totalServidores; i++){
        fwrite(ptrNotificacoes[i].pais,sizeof(char),20,arquivo);
        fwrite(ptrNotificacoes[i].cidade,sizeof(char),20,arquivo);
        fwrite(&ptrNotificacoes[i].serverUP,sizeof(int),1,arquivo);
        fwrite(&ptrNotificacoes[i].serverDOW,sizeof(int),1,arquivo);
    }//for 
    
    */
    fclose(arquivo); // fecha o arquivo de forma correta 

    free(ptrNotificacoes);//para liberar o espaço que foi alocado 
    ...
    // for para mostrar as informacoes
    for (int i = 0; i < totalServidores; i++) { 
        printf("| %-15s | %-15s | %-8d | %-8d |\n",
               ptrNotificacoes[i].pais,
               ptrNotificacoes[i].cidade,
               ptrNotificacoes[i].serverUP,
               ptrNotificacoes[i].serverDOW);
    }//for
